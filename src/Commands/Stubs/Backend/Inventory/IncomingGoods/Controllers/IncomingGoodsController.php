        ...
        try {
            ...
            if ($request->status == 'reject') {
                ...
                foreach ($data->details as $detail) {
                    ...
                    $mutationIn = $mutation->mutationIn($itemId, $data->warehouse_destination_id, abs($qty), now(), $unitPrice, $data->number, $data->company_destination_id, null, "Incoming Goods");
                    if ($mutationIn->getStatus() != 'success') {
                        throw new Exception($mutationIn->getErrorMessage(), Response::HTTP_UNPROCESSABLE_ENTITY);
                    }
                    $data->stockMutations()->save($mutationIn->getData()->model);
                }
            }
